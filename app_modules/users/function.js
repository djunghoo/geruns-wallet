	...
	.done(function(data){
		...
		$.each(obj,function(i){	
			if(i!=='pagination'){
				...
				da+='<td class="text-center pr-0 pos-rel">'+x+'</td>';
				da+='<td class="pr-0 pos-rel ">'+obj[i].username+'</td>';				
				if(obj[i].prefix_name==null){
					da+='<td class="pr-0 pos-rel">'+obj[i].FirstName+' '+obj[i].LastName+'</td>';			
					}else{
					da+='<td class="pr-0 pos-rel">'+obj[i].prefix_name+''+obj[i].FirstName+' '+obj[i].LastName+'</td>';
				}
				da+='<td class="pr-0 pos-rel text-center">'+obj[i].title+'</td>';							
				da+='<td class="pr-0 pos-rel text-center" id="status_'+obj[i].id+'">'+obj[i].utitle+'</td>';							
				da+='<td> ';
				if(obj[i].status=='2'){ sel='checked';}else{sel='';}
				//action buttons
				...
				da+='Edit';
				da+='</a>';
				da+='<a style="cursor:pointer" onClick="reset_password('+obj[i].id+')" class="dropdown-item">';
				da+='<i class="fa fa-key text-warning-m1 mr-1 p-2 w-4"></i>';
				da+='รีเซ็ทรหัสผ่าน';
				da+='</a>';							
				...
				//da+='<span class="text-info" style="cursor:pointer;" onClick="edit_data('+obj[i].id+')" title="แก้ไข"><i class="fa fa-edit">  </i>     </span>';		
				//da+='<span class="text-info" style="cursor:pointer;" onClick="edit_data('+obj[i].id+')" title="แก้ไข"><i class="fa fa-edit"> | </i>     </span>';	
				//da+='<span class="text-danger" style="cursor:pointer;" onClick="delete_data('+obj[i].id+')" title="ลบ"><i class="fa fa-trash"></i></span>';					
...
function update_status(id){
	//alert(id);
	if($('#st_'+id).prop("checked")){ st='2'; label='อนุมัติแล้ว'	}else{st='3'; label='ระงับการใช้งาน';}
	let param=new FormData();
	param.append('id',id);
	param.append('user_status',st);
	param.append('form_action',2);
	$.post(folder+'db.php',param)
	.done(function(data){
		//alert(data);
		if(data=='0'){
			$('#status_'+id).html(label);
			ms="<div class='bgc-success-d1 text-white px-3 pt-3'>";
			ms+="<div class='border-2 brc-white px-3 py-25 radius-round'>";
			ms+="<i class='fa fa-check text-150'></i>";
			ms+="</div></div>";
...
function check_email(){
	da="<div class='bgc-red-d1 text-white px-3 pt-3'>";
	da+="<div class='border-2 brc-white px-3 py-25 radius-round'>";
	da+="<i class='fa fa-times text-150'></i>";
	da+="</div></div><div class='p-3 mb-0 flex-grow-1'>";
	da+="<h4 class='text-130'>พบข้อผิดพลาด</h4>";
	da+="E-Mail Address ซ้ำครับ";
	da+="</div>";
	da+="<button data-dismiss='toast' class='align-self-start btn btn-xs btn-outline-grey btn-h-light-grey py-2px mr-1 mt-1 border-0 text-150'>&times;</button>";
	let param=new FormData();	
...
function check_pid(pid){
	$.post('/login/check_pid.php',{pid:pid}).done(function(data){
		//alert(data);
		if(data>0){
			// copy 10
			$.aceToaster.add({
				...
				<i class='fas fa-exclamation text-150'></i>\
				</div>\
				</div>\
				<div class='p-3 mb-0 flex-grow-1'>\
				<h4 class='text-130'>System Message</h4>\
				เลขประจำตัวประชาชนซ้ำครับ\
				</div>\
			...
			$.aceToaster.add({
				...
				<div class='p-3 mb-0 flex-grow-1'>\
				<h4 class='text-130'>System Message</h4>\
				เลขประจำตัวประชาชนไม่ถูกต้องครับ\
				</div>\
				<button data-dismiss='toast' class='align-self-start btn btn-xs btn-outline-grey btn-h-light-grey py-2px mr-1 mt-1 border-0 text-150'>&times;</button>",
...
function sub_select(source,target,sta){
	let param=new FormData();	
	param.append(source,$('#'+source).val());	
	param.append('status',sta);	
	param.append('form_action',9);
	$('#'+target).empty();
	$('#'+target).append('<option value="">กรุณาเลือก</option>');
	$.post(folder+'db.php',param)
	.done(function(data){
		$('#result').html(data);
		//alert(data);
		obj = jQuery.parseJSON(data);
		$.each(obj,function(i){
			$('#'+target).append('<option value="'+obj[i].schoolid+'">'+obj[i].schoolid+' : '+obj[i].schoolname+'</option>');
		});
...
$(function(){
	$('#schoolid').change(function(){
		sub_select('schoolid','schoolid2',2);
	});
	$('#s_schoolid').change(function(){
		sub_select('s_schoolid','s_schoolid2',2);
	});
	$('#username').blur(function(){ check_email();});
	...
	$('#frm-main').validate({
		...
		messages: {
			schoolid : {
				required: "กรุณาเลือกจังหวัดผู้ใช้งานด้วยครับ",
			},
			teacher_type_id: {
				required: "กรุณาเลือกประเภทผู้ใช้งานด้วยครับ",
			},
			prefix_id: {
				required: "กรุณาเลือกคำนำหน้าด้วยครับ",
			},
			FirstName: {
				required: "กรุณาพิมพ์ชื่อด้วยครับ",
			},
			LastName: {
				required: "กรุณาพิมพ์นามสกุลด้วยครับ",
			},
			PersonID: {
				required: "กรุณาพิมพ์เลขบัตรประชาชนด้วยครับ",
			},
			username: {
				required: "กรุณาพิมพ์อีเมล์ด้วยครับ",
				email:"กรุณาพิมพ์อีเมล์ด้วยครับ",
			},
			teacher_sid: {
				required: "กรุณาเลือกสถานะการทำงานด้วยครับ",
			},
		},
